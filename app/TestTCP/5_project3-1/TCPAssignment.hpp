...
protected:
	...
	virtual void write_packet(Packet * pck, uint8_t flagfield, uint32_t src_ip, uint32_t dest_ip,
											...
											uint32_t seq_no, uint32_t ack_no, int size = 0, int offset = 0, void * data = NULL,uint16_t window_size= htons(0xc800)) final;
	virtual void close_r(Sock * sc) final;
	virtual void update_arrived_seq(Sock * sock) final;
	virtual int read_from_pending(Sock * sock, void * target_buffer, int read_bytes) final;
};
